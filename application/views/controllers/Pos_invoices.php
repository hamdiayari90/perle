...
class Pos_invoices extends CI_Controller
{
    ...
    public function action()
    {
                ...
                if (is_array($pid)) {
                    foreach ($pid as $key => $value) {
                        ...
                        if ($product_id[$key] > 0 and $this->common->zero_stock()) {
                            $arome = $this->products_model->get_arome($product_id[$key]);
                            if($arome) {
                                if($this->aromes_model->not_available($arome['id'], $product_id[$key])) {
                                    echo json_encode(array('status' => 'Error', 'message' => 'Stock arome insuffisant'));
                                    $transok = false;
                                    $st_c = 1;
                                } else {
                                    $this->aromes_model->decrement_stock($arome['id'], $product_id[$key], $amt);
                                }
                            } else {
        ...
        $datapay=array();
        if($this->input->post('p_method')=="sodexespece"){
            $datapay["montantcash"]=$this->input->post('montantcash');
            $datapay["montantsodexo"]=$this->input->post('montantsodexo');
        }

                if ($pamnt > 0) $this->billing->paynow($invocieno, $pamnt, $tnote, $pmethod, $this->aauth->get_user()->loc, $bill_date, $account,$datapay);
...
//draft
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoicess) {
            $name="";
            $productNames= $this->invocies->invoice_products_perso($invoicess->id);
            foreach ($productNames as $productName) {
                $name.=$productName['product']."<br/> ";
            }
            $amountTotal+=$invoicess->total;
            $no++;
            $row = array();
            $row[] = $no;
            $row[] = '<a href="' . base_url("pos_invoices/view?id=$invoicess->id") . '">&nbsp; ' . $invoicess->tid . '</a>';
            $row[] = $name;
            $row[] = dateformat($invoicess->invoicedate);
            $row[] = amountExchange($invoicess->total, 0, $this->aauth->get_user()->loc);
            $row[] = '<span class="st-' . $invoicess->status . '">' . $this->lang->line(ucwords($invoicess->status)) . '</span>';
            $row[] = '<a href="' . base_url("pos_invoices/view?id=$invoicess->id") . '" class="btn btn-success btn-sm" title="View"><i class="fa fa-eye"></i></a>&nbsp;<a href="' . base_url("pos_invoices/thermal_pdf?id=$invoicess->id") . '&d=1" class="btn btn-info btn-sm"  title="Download"><span class="fa fa-download"></span></a>&nbsp;<a href="#" data-object-id="' . $invoicess->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
        ...
        if ($amountTotal>0) {
            ...
            $row[] = "";
            $row[] = "";
            $row[] ="Total : <b>". amountExchange($amountTotal, 0, $this->aauth->get_user()->loc)."</b>";
            $row[] = "";
            $row[] = "";
            $data[] = $row;
        }
    ...
    public function editaction()
    {
        ...
        if ($ptype == 4) {
            ...
            if ($this->db->update('geopos_invoices', $data)) {
                ...
                $this->billing->paynow($invocieno, $diff, $tnote, $pmethod, $this->aauth->get_user()->loc, 0, $account,$datapay);
                $this->registerlog->update($this->aauth->get_user()->id, $r_amt1, $r_amt2, $r_amt3, 0, $c_amt);
                if ($promo_flag) {
                    $cqty = $result_c['available'] - 1;
                    if ($cqty > 0) {
            ...
            if ($this->input->post('restock')) {
                foreach ($this->input->post('restock') as $key => $value) {
                    ...
                    if ($prid > 0) {
                        $arome = $this->products_model->get_arome($product_id[$key]);
                        if($arome) {
                            $this->aromes_model->increment_stock($arome['id'], $product_id[$key], $dqty);
                        } else {
