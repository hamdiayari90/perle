...
class Billing_model extends CI_Model
{
    ...
    public function paynow($tid, $amount, $note, $pmethod, $loc = false,$bill_date=null,$account_d=0,$datapay=array())
    {
        $account['id'] = false;
        if ($loc) {
        ...
        if(count($datapay)>0){
            $data["montantcash"]=$datapay["montantcash"];
            $data["montantsodexo"]=$datapay["montantsodexo"];
        }
        $this->db->trans_start();
