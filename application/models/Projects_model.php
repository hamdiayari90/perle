...
class Products_model extends CI_Model
{
    ...
    {
        $id_house=$this->aauth->get_user()->loc;
        $this->db->select('geopos_products.*,geopos_product_cat.title AS c_title,geopos_warehouse.title,sum(geopos_products.qty) as qtyTotal');
        $this->db->from($this->table);
        $this->db->join('geopos_warehouse', 'geopos_warehouse.id = geopos_products.warehouse');
        $this->db->join('geopos_product_cat', 'geopos_product_cat.id = geopos_products.pcat');
        $this->db->where("geopos_warehouse.loc = $id_house");
        if($id>0)
        ...
        if ($this->aauth->get_user()->loc) {
            if ($ware_valid['loc'] == $this->aauth->get_user()->loc OR $ware_valid['loc'] == '0' OR $warehouse == 0) {
                ...
                if ($this->db->insert('geopos_products', $data)) {
                    ...
                    $this->aauth->applog("[New Product] -$product_name  -Qty-$product_qty ID " . $pid,
    ...
    public function get_arome($pid)
    {
        ...
        $this->db->where('id', $product['arome_id']);
        $query = $this->db->get();
        ...
        if ($this->aauth->get_user()->loc) {
            if ($ware_valid['loc'] == $this->aauth->get_user()->loc OR $ware_valid['loc'] == '0' OR $warehouse == 0) {
                ...
                if ($this->db->update('geopos_products')) {
                    ...
                    $this->aauth->applog("[Update Product] -$product_name  -Qty-$product_qty ID " . $pid,
                        $this->aauth->get_user()->username);
                    echo json_encode(array(
                        'status' => 'Success',
                        'message' =>
...
FROM geopos_products $whr");
    ...
    public function transfer($from_warehouse, $products_l, $to_warehouse, $qty)
    {
        ...
        foreach ($products_l as $row) {
            ...
            if ($pr["pcat"] == 1) {
                $this->db->from('geopos_aromes');
                $this->db->where('id', $pr["arome_id"]);
                $query = $this->db->get();
                $arome = $query->row_array();
                if ($arome["stock"] > $qty) {
                    // reduire taille ancien
                    $this->db->set('stock', "stock-" . $qty, false);
                    $this->db->where('id', $pr["arome_id"]);
                    $this->db->update('geopos_aromes');
                    // update product
                    $this->db->set('qty', $arome['stock'] . '/stock_variant', false);
                    $this->db->where('arome_id', $arome["id"]);
                    $this->db->update('geopos_products');

                    $this->db->from('geopos_aromes');
                    $this->db->where('code', $arome["code"]);
                    $this->db->where('warehouse', $to_warehouse);
                    $query = $this->db->get();
                    $aromeAdedMl = $query->row_array();
                    if (!is_null($aromeAdedMl)) {
                        $this->db->set('stock', "stock+" . $qty, false);
                        $this->db->where('id', $aromeAdedMl["id"]);
                        $this->db->update('geopos_aromes');
                        
                        $qtytotal=$aromeAdedMl['stock']+$qty;
                        $this->db->set('qty', $qtytotal . '/stock_variant', false);
                        $this->db->where('arome_id', $aromeAdedMl["id"]);
                        $this->db->update('geopos_products');

                    } else {
                        $aromeAdd = array();
                        $aromeAdd["label"] = $arome["label"];
                        $aromeAdd["code"] = $arome["code"];
                        $aromeAdd["variants"] = $arome["variants"];
                        $aromeAdd["stock_variants"] = $arome["stock_variants"];
                        $aromeAdd["stock"] = $qty;
                        $aromeAdd["buying_price"] = $arome["buying_price"];
                        $aromeAdd["saling_price"] = $arome["saling_price"];
                        $aromeAdd["warehouse"] = $to_warehouse;
                        $this->db->insert('geopos_aromes', $aromeAdd);
                        $idArome = $this->db->insert_id();
                        // select products
                        $this->db->from('geopos_products');
                        $this->db->where('arome_id', $pr["arome_id"]);
                        $query = $this->db->get();
                        ...
                        foreach ($products as $index => $product) {
                            $new_products[] = array(
                                ...
                                'stock_variant' => $product["stock_variant"],
                                'qty' => round($qty / $product["stock_variant"]),
                                'merge' => 0,
                                'sub' => 0,
                                'vb' => 0,
                            );
                        ...
                        // insert
                        $out = $this->db->insert_batch('geopos_products', $new_products);
                    }
                }
    ...
    // code here
                    ...
                    // Add transferred quantity to the product's quantity in the "to" warehouse
                    if (isset($pr['product_code'])) {
                        $c_pid = $pr['product_code'];
                        $product_name = $pr2['product_code'];
                        $this->db->set('qty', "qty+$qty", false);
                        $this->db->where('product_code', $c_pid);
                        $this->db->where('warehouse', $to_warehouse);
                        $this->db->update('geopos_products');
                        $this->movers(1, $c_pid, $qty, 0, 'Stock Transferred W ' . $to_warehouse_name);
                        $this->aauth->applog("[Product Transfer] -$product_name  -Qty-$qty W $to_warehouse_name  ID " . $c_pid,
                            $this->aauth->get_user()->username);
                    } else {
                        $product_name = $pr2['product_code'];
                        $this->db->insert('geopos_products', $data);
                       ...
                       $this->movers(1, $pid, $qty, 0, 'Stock Transferred & Initialized W ' . $to_warehouse_name);
                        $this->aauth->applog("[Product Transfer] -$product_name  -Qty-$qty  W $to_warehouse_name ID " . $pr2['pid'],
                            $this->aauth->get_user()->username);
                    }
                
                    // Subtract transferred quantity from the product's quantity in the "from" warehouse
                   $this->db->set('qty', "qty-$qty", false);
                        $this->db->where('product_code', $c_pid);
           ...
           $this->movers(1, $row, $qty, 0, $to_warehouse_name);
            $i++;
        }
