...
$flag_p=true;
            $bar = " (geopos_products.barcode = '" . (substr($name, 0, -1)) . "' OR geopos_products.barcode LIKE '" . $name . "%')";
               ...
               $query = "SELECT geopos_products.*  FROM geopos_products $join WHERE " . $qw . "$bar AND (geopos_products.qty>0) ORDER BY geopos_products.product_name
               GROUP BY geopos_products.product_code
               LIMIT 6";
            ...
            if ($billing_settings['key1'] == 2) {
                ...
                $query = "SELECT geopos_products.*,geopos_product_serials.serial FROM geopos_product_serials  LEFT JOIN geopos_products  ON geopos_products.pid=geopos_product_serials.product_id $join WHERE " . $qw . "geopos_product_serials.serial LIKE '" . strtoupper($name) . "%'  AND (geopos_products.qty>0)
                GROUP BY geopos_products.product_code
                LIMIT 18";

            } else {

                $query = "SELECT geopos_products.* $e FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' $bar OR geopos_products.product_code LIKE '" . strtoupper($name) . "%') AND (geopos_products.qty>0) 
                GROUP BY geopos_products.product_code
...
if($flag_p) {
    ...
    $allNames=array();
    foreach ($result as $row) {
        $product_name=substr($row['product_name'],0,strrpos($row['product_name'],"SANS FIX"));
        if(!strrpos($row['product_name'],"SANS FIX"))
            $product_name=substr($row['product_name'],0,strrpos($row['product_name']," "));
            $product_name_ml=substr($row['product_name'],strrpos($row['product_name']," "),strlen($row['product_name']));
            if(!strpos($product_name_ml,'ML')){
                $row['product_codes']=array($row['product_code']);
                $dataAll[]=$row;
            }
            else{
                $nameProduct=substr($product_name,-3,3);
                if(substr_count($nameProduct, ' ') === 2 ){
                    $row['product_name']=substr($product_name,0,-3);
                    if(array_key_exists(substr($product_name,0,-3), $dataAll) ){
                        array_push($dataAll[substr($product_name,0,-3)]['product_codes'],$row['product_code']);
                    }
                    else{
                        $row['product_codes']=array($row['product_code']);
                        $dataAll[substr($product_name,0,-3)]=$row;
                    }
                }
                else{
                    if(array_key_exists(substr($product_name,0,-1), $dataAll) ){
                        array_push($dataAll[substr($product_name,0,-1)]['product_codes'],$row['product_code']);
                    }
                    else{
                        $row['product_codes']=array($row['product_code']);
                        $dataAll[substr($product_name,0,-1)]=$row;
                    }
...
//        $allNames[]=$row['product_name'];
    ...
    foreach ($dataAll as $row) {
        ...
        $out .= '    <div class="col-2 border mb-1 mounir "  ><div class=" rounded" >
                                 <a  id="posp' . $i . '"  class="' . $p_class.$modalValid . ' round" data-toggle="modal" data-target="#myModal' . $i . '"   data-name="' . $product_name . '"  data-price="' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . amountFormat_general($row['taxrate']) . '"  data-discount="' . amountFormat_general($row['disrate']) . '" data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . amountFormat_general($row['qty']) . '" data-unit="' . $row['unit'] . '" data-serial="' . @$row['serial'] . '" data-bar="' . $bar . '">
                                        <img class="round"
                                             src="' . base_url('userfiles/product/' . $row['image']) . '"  style="max-height: 100%;max-width: 100%">
                                        <div class="text-center" style="margin-top: 4px;">
                                       
                                            <small style="white-space: pre-wrap;">' .$product_name . '</small>
                                ...
                                if($row['pcat']==1){
                                    $out .= '               <div class="modal fade" id="myModal' . $i . '" role="dialog">
                                <div class="modal-dialog">
                                
                                  <!-- Modal content-->
                                  <div class="modal-content">
                                    <div class="modal-header">
                                      <h6 class="modal-title">Sélectionner la quantité</h6>
                                    </div>
                                    <div class="modal-body">';
                                    $ids = join("','",$row['product_codes']);   

                                    $query_qty = "SELECT * FROM geopos_products  WHERE geopos_products.warehouse=" .$row['warehouse']." and geopos_products.qty>0 and geopos_products.product_code IN ('".$ids."') ";
                                    $query_qty = $this->db->query($query_qty);
                                    $result_qty = $query_qty->result_array();
                                    $dataArray=array();

                                    foreach ($result_qty as $row_qty) {
                                        $product_name_last=substr($row_qty['product_name'],strrpos($row_qty['product_name']," "),strlen($row_qty['product_name']));
                                        if(!in_array($product_name_last,$dataArray)){
                                        $out.=' <a  id="posp' . $i . '"  class="' . $p_class . ' round ml_quantite" data-toggle="modal" data-target="#myModal' . $i . '"   data-name="' . $row_qty['product_name'] . '"  data-price="' . amountExchange_s($row_qty['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . amountFormat_general($row_qty['taxrate']) . '"  data-discount="' . amountFormat_general($row_qty['disrate']) . '" data-pcode="' . $row_qty['product_code'] . '"   data-pid="' . $row_qty['pid'] . '"  data-stock="' . amountFormat_general($row_qty['qty']) . '" data-unit="' . $row_qty['unit'] . '" data-serial="' . @$row_qty['serial'] . '" data-bar="' . $bar . '">
                                            <small style="white-space: pre-wrap;">' .$product_name_last . '</small>
                                            </a>';
                                            $dataArray[]=$product_name_last;
                                    }
                                }
