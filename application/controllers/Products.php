...
class Products extends CI_Controller
{
    ...
    public function product_list()
    {
        ...
        foreach ($list as $prd) {
            $product_name=substr($prd->product_name,0,strrpos($prd->product_name,"SANS FIX"));
            if(!strrpos($prd->product_name,"SANS FIX"))
                $product_name=substr($prd->product_name,0,strrpos($prd->product_name," "));
            $product_name_ml=substr($prd->product_name,strrpos($prd->product_name," "),strlen($prd->product_name));
            ...
            $row[] = $no;
            $pid = $prd->pid;
            $row[] = '<a href="#" data-object-id="' . $pid . '" class="view-object"><span class="avatar-lg align-baseline"><img src="' . base_url() . 'userfiles/product/thumbnail/' . $prd->image . '" ><br/></span>&nbsp;' . $product_name . '</a>';
            $arome = $this->products->get_arome($prd->pid);
            $row[] = $arome? '(Arome: '.$arome['stock'].'gr)':+$prd->qtyTotal;
...
&nbsp;<a href="' . base_url() . 'products/edit?id=' . $pid . '"  class="btn btn-purple btn-sm"><span class="fa fa-edit"></span>' . $this->lang->line('Edit') . '</a><div class="dropdown-divider"></div>&nbsp;<a href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm  delete-object"><span class="fa fa-trash"></span>' . $this->lang->line('Delete') . '</a>
    ...
    public function view_over()
    {
        ...
        if($product["pcat"]== 1 ){
            ...
            $this->db->where('geopos_products.arome_id', $product["arome_id"]);
            $this->db->where('geopos_products.warehouse', $product["warehouse"]);
            $query = $this->db->get();
...
//if(!$cat_ware) exit();
    ...
    public function transfer()
    {
        ...
        $this->load->view('products/transfer');
        $this->load->view('fixed/footer');
    }
    ...
    public function delete_all_kheyro()
    {
        ...
        $this->db->empty_table('geopos_products');
    }
