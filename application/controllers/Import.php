...
//print_r($sheetData);
        ...
        if($pcat == 1) { //1 category aromes
            $aromes = array();
            $this->load->model('aromes_model');
            $sheetData = array_slice($sheetData,1); // remove first head row
            foreach ($sheetData as $row) {
                $arome_id = $this->aromes_model->create($row[0],$row[1], $this->floatvalue($row[2]),$row[3],(float)str_replace(',', '.', str_replace('.', '', $row[4])),(float)str_replace(',', '.', str_replace('.', '', $row[5])),$row[6],$warehouse);
                $variants = explode(';', $row[3]);
                $stock_variants = explode(';', $row[6]);
                $price_variants = explode(';', $row[7]);
                for($i=0, $iMax = count($variants); $i < $iMax; $i++) {
                    $barcode = rand(100, 999) . '-' . rand(0, 9) . '-' . rand(1000000, 9999999) . '-' . rand(0, 9);
                    $variant=str_replace('SANS FIX ','',$variants[$i]);
                    $products[] = array(
                        'pid' => null,
                        'pcat' => $pcat,
                        'warehouse' => $warehouse,
                        'product_name' => $row[1].' '.$variant.'ML',
                        'product_code' => $row[0],
                        'product_price' => (float)str_replace(',', '.', str_replace('.', '', $price_variants[$i])),
                        'barcode' => $barcode,
                        'arome_id' => (int)$arome_id,
                        'variant' => $variant,
                        'stock_variant' => (float)str_replace(',', '.', str_replace('.', '', $stock_variants[$i])),
                        'qty' => (int)((float)str_replace(',', '.', str_replace('.', '', $row[2])) / (float)str_replace(',', '.', str_replace('.', '', $stock_variants[$i]))),
                        'merge' => 0,
                        'sub' => 0,
                        'vb' => 0,
                    );
